import type { Project, Workspace, MindMapNode, MindMapEdge } from "./types";

// For Vercel deployment: use VERCEL_URL or fall back to localhost for development
function getBaseUrl(): string {
  // In production on Vercel, use the deployment URL
  if (process.env.VERCEL_URL) {
    return `https://${process.env.VERCEL_URL}`;
  }
  // Custom API URL if provided
  if (process.env.MINDFRAME_API_URL) {
    return process.env.MINDFRAME_API_URL;
  }
